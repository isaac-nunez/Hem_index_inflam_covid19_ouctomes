---
title: "Marcadores de inflamación en pacientes con covid19"
output:
...
**Objetivo del estudio**: relación entre marcadores inflamatorios al ingreso hospitalario en pacientes con covid-19 y desenlaces clínicos. Secundario. ¿qué tan útil es pedirlos a cada rato? Sería esperado encontrar lo mismo en pacientes con otras enfermedades agudas?</br>
**Otra idea**es agregar a controles que tengan neumonía por otro motivo (aunque probablemente el innsz no sea un buen lugar, ya que esos controles tendrán comorbilidades que afecten la inflamación de los pacientes)
</br>
**Metodología del estudio**</br>
-Recopilar TODOS los marcadores, porque todos son potencialmente "útiles".</br>
-Se determinará cuál es el riesgo de cada uno respecto al desenlace de interés (ingreso a UTI y/o mortalidad).</br>
-Se intentará determinar un punto de corte para cada uno con curvas ROC. </br>
-Se intentará determinar si una combinación de todos vs BH(con los índices estipulados, en caso de que resulten relevantes en el análisis univariado) si no, sólo con leucos/linfos totales Y pcr (la cual en teoría debería estar disponible en todas partes).</br>
-En caso de que las curvas ROC resulten similares, se determinará por medio de un criterio de información (Ej. Akaike) si realmente vale más la pena usar todos los marcadores o si es relativamente igual el resultado de usar uno u otro.</br>

**NOTAS**  
Cómo decidimos la manera en la cual usaremos los marcadores?  
Si este estudio es de cinética, sería algo muy diferente a usarlo de curva roc. Quizá si hacemos la curva roc justo al ingreso y el resto del papaer lo enfocamos en la cinética que llevan los marcadores.</br>
-HACER CURVAS CON LA MEDIANA DEL MARCADOR PARA ESE DÍA Y LUEGO PARA EL RESTO. Checar este paper.
https://journals.lww.com/anesthesia-analgesia/FullText/2014/09000/C_Reactive_Protein_Kinetics_After_Major_Surgery.19.aspx  
-Al parecer está desactivada la opción de descargar la base de un laboratorio directo desde labsis. Dejaré eso pendiente. 


**Cargando los paquetes y bases correspondientes**
...
library(patchwork);library(stddiff);library(flextable);library(knitr);library(pROC)
...
#Aquí abajo cargo el resto de los labs por si llegara a necesitar alguno más adelante (los labs de ingreso ya los integraron en la base principal)
...
#Esta base la estoy agregando porque la base que me pasaron hoy tiene muchas fechas de urgencias faltantes
...
En este punto, se hará un EDA con énfasis en que las variables estén correctamente codificadas (rango de fechas, que los valores estén correctamente introducidos, etc). Esto probablamente tenga que dejarlo para después por fines de tiempo, pero es importante que lo cheque, porque a lo que he visto es altamente probable que se hayan cometido múltiples errores en cuanto al formato de las variables que se introducen en las casillas. </br>
</br>
**Cuáles marcadores incluír?**</br>
-Dímero D  
-Fibrinógeno  
-PCR  
-CPK  
-DHL  
-Ferritina  
-Índices hematológicos (neu/linf, plaq/linf) etc etc

**¿Cuáles son los registros que no tienen una fecha de ingreso o que no tienen una fecha de ingreso a urgencias posterior a hospitalización, por ejemplo**  
Hay muchas fechas de ingreso a urgencias que están faltantes, aquí uniré la base nueva con la anterior de manera que se puedan recuperar algunos de ellos.
...
covid_full_4 <- covid_full_3 %>% 
  mutate(nlymr = round(neutrofilos_por_ingreso/linfocitos_por_ingreso,2),#neutrophil lymphocyte ratio
         ...
         linfocitos_totales = round((leucocitos_ingreso*1000)*(linfocitos_por_ingreso/100),0))#ESTA VARIABLE CAMBIARÁ CUANDO HAGA LOS MODELOS DE RIESGO EN COMPETENCIA (si es que los llego a hacer por cuestiones de tiempo)
```


Modelos de riesgo   
**OJO**: esta es una versión rápida y sucia, no he hecho DAGs ni un ejercicio similar, entonces podría cambiar. En este momento creo que no valdría la pena hacerlos ajustados ya que seguramente muchos se correlacionarían y probablemente esté sobreajustando. 

En esta el producto final son las gráficas de los ajustes por splines para los índices hematológicos. 
...
nlymr_spline_plot <- ggplot(nlymru_df, aes(x = nlymr_count, y = exp(mean_ci - centernlymr_uv))) +
  ...
  xlab("Neutrophil-lymphocite index at admission")+
  theme_light()+
...
lmr_spline_plot <- ggplot(lmru_df, aes(x = lmr_count, y = exp(mean_ci - centerlmr_uv))) +
  ...
  ylab("HR of ICU admission or death") +
  xlab("Lymphocite-monocite index at admission")+
  theme_light()+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_blank(),
...
plr_spline_plot_mv <- ggplot(plrmv_df, aes(x = plr_count, y = exp(mean_ci - centerplr_mv))) +
  ...
  ylab("") +
  xlab("Platelet-lymphocite index at admission")+
  theme_light()+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_blank(),
...
lmr_spline_plot_mv <- ggplot(lmrmv_df, aes(x = lmr_count, y = exp(mean_ci - centerlmr_mv))) +
  ...
  ylab("HR of ICU admission \n or death") +
  xlab("Lymphocite-monocyte index at admission")+
  theme_light()+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_blank(),
...
#UNIVARIADOS CON SPLINES PARA DHL#######PROBABLEMENTE DHL PUEDA SER AJUSTADA A UN MODELO LINEAR
...
#UNIVARIADOS CON SPLINES PARA FERRITINA, por lo pronto voy a retirar los valores demasiado grandes ( y unos que al parecer están negativos)
...
uni_ferr <- coxph(Surv(sobrevida_primer_evento, desenlace_uti == 1) ~ rcs(ferritina_ingreso,3), data = filter(covid_full_4, ferritina_ingreso <=3000))
...
pcr_univar <- pcru_df[with(pcru_df, pcr_count == 0.5 | pcr_count == 0.94 | pcr_count == 5.04 | 
              pcr_count == 10.03 | pcr_count== 15.02),] %>% 
...
#UNIVARIADOS CON SPLINES PARA DÍMERO D, al parecer solo 3 tienen valores arriba de 10000, por lo pronto veremos como se comporta con estos valores pero probablemente los quite porque el intervalo será demasiado ancho
#fivenum(covid_full_4$dimerod_ingreso)
#boxplot(covid_full_4$dimerod_ingreso)

uni_dd <- coxph(Surv(sobrevida_primer_evento, desenlace_uti == 1) ~ rcs(dimerod_ingreso,3), data = filter(covid_full_4, dimerod_ingreso <5000))
...
dd_univar <- ddu_df[with(ddu_df, dd_count == 209 | dd_count == 499 | dd_count == 1007 | 
              dd_count == 1466 | dd_count== 2531),] %>% 
...
mv_ferr <- coxph(Surv(sobrevida_primer_evento, desenlace_uti == 1) ~ rcs(ferritina_ingreso,3) +
                  ...
                  data =  filter(covid_full_4, ferritina_ingreso <=3000))
...
mv_pcr <- coxph(Surv(sobrevida_primer_evento, desenlace_uti == 1) ~ rcs(ferritina_ingreso,3) +
                    edad + sexo + imc + diabetes + hipertension + sato2_aa, 
                  data =  covid_full_4)

coxpcr_mv <- termplot(uni_pcr, se=TRUE, plot = F)$pcr_ingreso
centerpcr_mv <- with(coxpcr_mv, y[x == 10.04])
...
pcr_mv <- pcrmv_df[with(pcrmv_df, pcr_count == 0.5 | pcr_count == 1.07 | pcr_count == 5.04 | 
              pcr_count == 10.03 | pcr_count== 15.02),] %>% 
...
mv_dd <- coxph(Surv(sobrevida_primer_evento, desenlace_uti == 1) ~ rcs(dimerod_ingreso,3) +
                    edad + sexo + imc + diabetes + hipertension + sato2_aa, 
                  data = filter(covid_full_4, dimerod_ingreso <5000))
...
dd_mv <- ddmv_df[with(ddmv_df, dd_count == 209 | dd_count == 499 | dd_count == 1007 | 
              dd_count == 1466 | dd_count== 2531),] %>% 
...
La idea hasta ahora es incluir cinco columnas en la tabla:</br>
1-Nombres de las variables</br>
2-Valores de la base en general</br>
3-Valores de los que NO ingresaron a UCI</br>
4-Valores de los que SI ingresaron a UCI</br>
5-Diferencias estandarizadas</br>

Podría ser conveniente también incluir una columna para los que se murieron sin entrar a la UCI
```{r Primeras funciones de la tabla,  warning = F, include = F,message=F}
#vectores a incluir en la tabla
#RECORDAR QUE LA BASE ES covid_full_4
generalisimo <- function(x){
  c(n_distinct(x[,1]),  
    round(median(x$sobrevida_primer_evento,1)),
    NA, 
    sum(x$sexo == "m"), 
    ...
    median(x$neutrofilos_por_ingreso, na.rm = T),
    median(x$linfocitos_por_ingreso, na.rm = T),
    median(x$monocitos_por_ingreso, na.rm = T),
...
En el siguiente chunk coloco las funciones de:</br>
-Porcentajes (para las variables categóricas)</br>
-Diferencias estandarizadas (para la última columna)</br>

```{r funciones para la tabla 1, include = F,message=F}
...
variables_p <-c(NA, "sobrevida_primer_evento", NA, "mujer", "edad", "imc", "diabetes" , "hipertension", 
                ...
                NA, "hb_ingreso", "plaquetas_ingreso", "leucocitos_ingreso", "neutrofilos_por_ingreso",
                "linfocitos_por_ingreso","monocitos_por_ingreso", "nlymr", "dnlr", "plr", "lmr",  NA, "pcr_ingreso",
...
Impresión de la tabla 1
```{r Tabla 1, echo = F, warning=F,message=F}
tabla_1_df <- data.frame(Variable = variables,
                         'Toda la cohorte' = str_c(generalisimo(covid_full_4)," ", porcentajes_iqr_gral),   
                         'No ingresaron a UTI' = str_c(generalisimo(covid_full_4 %>% filter(desenlace_uti == F)),
                                                       " ", porcentajes_iqr_nouti),
                         'Ingresaron a UTI' = str_c(generalisimo(covid_full_4 %>% filter(desenlace_uti == T)),
                                                    " ", porcentajes_iqr_uti),
...
```{r Tabla 2, echo  = F,warning=F,message=F}
variables_tabla_2 <- c("Índice neutro-linfo", c(2.5, 5, 10, 15, 20), NA,
                       ...
                       "PCR", c(0.5, 1, 5, 10, 15), NA,
                       "Dímero D", c(200, 500, 1000, 1500, 2500), NA,
...
```{r guardar la base de trabajo y calculos para manuscrito, include = F}
#write.csv(covid_full_4, file = "base_de_trabajo.csv")
save(covid_full_4, file = "covid_full_4.Rda")

range(covid_full_4$fecha_ing_urg)

base_trabajo <- filter(covid_full_4, fecha_ing_urg >= "2020-03-21")

#Porcentaje que tenían diabetes o hipertensión
sum(base_trabajo$hipertension == T | base_trabajo$diabetes == T)
sum(base_trabajo$hipertension == T | base_trabajo$diabetes == T)/nrow(base_trabajo)

#Porcentaje que tenían algún síntoma
sum(base_trabajo$disnea == T | base_trabajo$tos == T | base_trabajo$fiebre == T |
      base_trabajo$dolor_toracico == T | base_trabajo$cefalea == T |base_trabajo$mialgias == T |
      base_trabajo$diarrea == T )

#Índices hematológicos (anemia, leucocitosis / leucopenia, plaquetopenia)
nrow(filter(base_trabajo, linfocitos_totales<1000))
nrow(filter(base_trabajo, leucocitos_ingreso>10))
nrow(filter(base_trabajo, hb_ingreso<12))
nrow(filter(base_trabajo, plaquetas_ingreso<150))
nrow(filter(base_trabajo, plaquetas_ingreso<100))

#Marcadores inflamatorios
nrow(filter(base_trabajo, pcr_ingreso >1))
nrow(filter(base_trabajo, pcr_ingreso >1))/nrow(base_trabajo)


#Pacientes con un desenlace
nrow(filter(base_trabajo, muerte == T | ing_uti == T))
nrow(filter(base_trabajo, muerte == T | ing_uti == T | egreso == T))
nrow(filter(base_trabajo, muerte == T))
nrow(filter(base_trabajo, ing_uti == T))
nrow(filter(base_trabajo, muerte == T | ing_uti == T))/nrow(base_trabajo)
```

**Pendientes para esta sesión:**</br>
-Checar el chunk que marqué en su título porque hay registros que se están perdiendo ahí y ese chunk probablemente sea inecesario
-Modificar los modelos de riesgo debido a que los valores de los modelos de predicción se modificaron con los nuevos pacientes. 
-Redactarlo y enviarselo a Sergio, TODO ESTO DEBE QUEDAR PARA LA PRÓXIMA SESIÓN
